...
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
RETRO_DIR="${GSTACK_WORKSPACE_DIR:-$HOME/.ai-workspace/$REPO}/retros"
ls -t "$RETRO_DIR"/*.json 2>/dev/null
```
...
today=$(date +%Y-%m-%d)
existing=$(ls "$RETRO_DIR"/${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
# Save as $RETRO_DIR/${today}-${next}.json (~/.ai-workspace/<repo>/retros/${today}-${next}.json)
```
...
5. Save only the current-window snapshot to `~/.ai-workspace/<repo>/retros/` (same as a normal retro run); do **not** persist the prior-window metrics.
...
- Output directly to the conversation — do NOT write to filesystem (except the `~/.ai-workspace/<repo>/retros/` JSON snapshot)

## Important Rules

- ALL narrative output goes directly to the user in the conversation. The ONLY file written is the `~/.ai-workspace/<repo>/retros/` JSON snapshot.
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.gstack/retros/` (not the per-repo `~/.ai-workspace/<repo>/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
