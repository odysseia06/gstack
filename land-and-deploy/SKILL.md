...
```bash
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
REPORT_DIR="${GSTACK_WORKSPACE_DIR:-$HOME/.ai-workspace/$REPO}/deploy-reports"
mkdir -p "$REPORT_DIR"
$B snapshot -i -a -o "$REPORT_DIR/post-deploy.png"
```
...
Save report to `$REPORT_DIR/{date}-pr{number}-deploy.md` (`~/.ai-workspace/<repo>/deploy-reports/...`).
