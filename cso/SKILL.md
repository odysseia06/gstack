...
**Trend Tracking:** If prior reports exist in `~/.ai-workspace/<repo>/security-reports/`:
```
...
```bash
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
REPORT_DIR="${GSTACK_WORKSPACE_DIR:-$HOME/.ai-workspace/$REPO}/security-reports"
mkdir -p "$REPORT_DIR"
```

Write findings to `$REPORT_DIR/{date}-{HHMMSS}.json` (`~/.ai-workspace/<repo>/security-reports/...`) using this schema:
...
Security reports are written to the AI workspace (`~/.ai-workspace/<repo>/security-reports/`), outside the repo entirely, so they stay local and never get committed.
